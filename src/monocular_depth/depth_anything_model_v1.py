...
import numpy as np
import cv2, torch
model_path = os.path.join('external_src', 'monocular_depth', 'Depth-Anything')
sys.path.insert(0, model_path)
from depth_anything.dpt import DepthAnything as DepthAnythingBaseModel
from run_helper_functions import resize_image


class DepthAnythingV1Model(torch.nn.Module):
    '''
    Class for interfacing with DepthAnything V1 model
    ...
    def __init__(self, device=torch.device('cuda'), encoder='vitl', use_pretrained=True):
        ...
        if use_pretrained:
            self.model.load_state_dict(torch.load(f'{model_path}/model_ckpts/depthanything_v1/depth_anything_{encoder}14.pth'))
