...
sys.argv = [sys.argv[0]]
from omnidc_model import OMNIDCModel
...
def run_omnidc(
    ...
    rgb_ext: str = "png",
    device: str = "cuda",
    depth_multiplier: float = 256.0,
    save_npy: bool = False,     # kept for parity with original args (not used yet)
    ...
    Args:
        rgb_dir: folder containing RGB images (e.g. .../images or .../data/image)
        sparse_depth_dir: folder containing sparse depth 16-bit PNGs, filenames match RGB stems
        out_dir: output folder for predictions
        intrinsics_path: path to universal 3x3 intrinsics .npy (recommended). If omitted, uses identity.
        rgb_ext: RGB extension to match (png/jpg/jpeg)
        device: "cuda" or "cpu"
        depth_multiplier: multiplier used by your load_depth() (default 256)
        save_npy: also save float32 depth maps as .npy (not implemented in original script)
        overwrite: overwrite existing outputs

    Returns:
        model: OMNIDCModel instance
        out_dir: output directory (string)
    """
    rgb_dir = Path(rgb_dir)
    dep_dir = Path(sparse_depth_dir)
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    out_depth_png = out_dir / "depths"
    out_depth_png.mkdir(parents=True, exist_ok=True)

    rgb_paths = sorted(rgb_dir.glob(f"*.{rgb_ext}"))
    if len(rgb_paths) == 0:
        raise FileNotFoundError(f"No RGB images found in {rgb_dir} with extension .{rgb_ext}")

    dev = torch.device(device)
    to_tensor = T.ToTensor()

    # Model
    model = OMNIDCModel(device=dev)
    model.eval()

    if intrinsics_path is None:
        intrinsics = torch.eye(3, device=dev, dtype=torch.float32).reshape(1, 3, 3)
    else:
        K = data_utils.load_intrinsics(intrinsics_path, device=dev).cpu()  # expected shape (3,3)
        K = np.asarray(K, dtype=np.float32)
        intrinsics = torch.from_numpy(K).to(device=dev, dtype=torch.float32).reshape(1, 3, 3)

    for i, rgb_path in enumerate(rgb_paths):
        sparse_path = dep_dir / f"{rgb_path.stem}.png"
        if not sparse_path.exists():
            continue

        out_png_path = out_depth_png / f"{i+1:06d}.png"

        if (not overwrite) and out_png_path.exists():
            # keep original semantics; original code referenced `stem` (undefined),
            # so we use rgb_path.stem here but don't change behavior otherwise.
            print(f"[SKIP] {rgb_path.stem} (already exists)")
            continue

        img_np = data_utils.load_image(str(rgb_path))  # HWC float32 in [0,1]
        dep_np = data_utils.load_depth(str(sparse_path), multiplier=depth_multiplier)  # HW float32

        image = to_tensor(img_np).unsqueeze(0).to(dev)           # [1,3,H,W]
        sparse_depth = to_tensor(dep_np).unsqueeze(0).to(dev)    # [1,1,H,W]
        ...
        with torch.no_grad():
            pred = model.forward_depth(image, sparse_depth, validity_map, intrinsics)  # [1,1,H,W]
            pred_hw = (
                pred.squeeze(0)
                    .squeeze(0)
                    .detach()
        ...
        data_utils.save_depth(pred_hw, str(out_png_path))
...
def run_depth_eval(pred_depth_path,
    ...
    if use_mask:
        log(f"mask_dir={mask_path}", log_path)
    log('', log_path)
