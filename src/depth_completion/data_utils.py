...
def load_intrinsics(path: str, device: torch.device):
    """
    Load intrinsics from .npy and return [1,3,3] float32 on device.
    ...
    if K.shape == (3, 3):
        ...
        if not (0 <= index < K.shape[0]):
            raise IndexError(f"intrinsics index {index} out of range for {K.shape}")
        K_i = K[index]
    else:
        raise ValueError(f"Unsupported intrinsics npy shape: {K.shape} (expected (3,3) or (N,3,3))")

    return torch.from_numpy(K_i).to(device).reshape(1, 3, 3)
...
def load_text(text_path, dataset, select_random=True):
    ...
    try:
        ...
        raise RuntimeError(f"Could not decode {text_path} as UTF‑8")
