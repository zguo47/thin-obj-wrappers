...
class UniDepthV1(
    ...
    def forward(self, inputs, sensor_token=None, gt_intrinsics=None):
        H, W = inputs["image"].shape[-2:]
        rgbs = inputs["image"]
        # Encode
        encoder_outputs, cls_tokens = self.pixel_encoder(rgbs, None)
    ...
    @torch.no_grad()
    def infer(self, rgbs: torch.Tensor, sensor_token=None, grid_tensor=None,
                intrinsics=None, skip_camera=False):
    ...
    def build(self, config):
        ...
        # print(f"Input into decoder: {config}")
