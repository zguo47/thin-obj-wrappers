if __name__=='__main__':
    ...
    files = []
    # Get total number of samples where there is no Lidar sensor
    count = 0
    d_max = 0
    ...
    n_samples = len(level5data.sample)
    ...
    for l in range(len(sensor_channels)):
        ...
        for n in tqdm.tqdm(range(n_samples)):
            ...
            Image.fromarray(depth).save(depth_root + f'/depth_{n}_{l}.png', "PNG")
            # print("image saved at", depth_root + f'/depth_{n}_{l}.png')

            # rbg_path = rgb_root + f'/rgb_{n}.png'
            # depth_path = depth_root + f'/depth_{n}.png'
            rgb_path = f"lyft_data/rgb/rgb_{n}_{l}.png"
            depth_path = f"lyft_data/depth/depth_{n}_{l}.png"
    ...
    with open('splits/lyft/train_annotations.json', 'w') as f:
        json.dump(files_dict, f)
    print("json file saved at splits/lyft/train_annotations.json")
    print("max depth", d_max)