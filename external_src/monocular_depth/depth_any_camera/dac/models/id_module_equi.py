...
from dac.utils import (AttentionLayerIsoPE, PositionEmbeddingSine,
...
class ISDHead(nn.Module):
    def __init__(
        ...
        self.eps = eps
        self.pe_mlp = pe_mlp
        self.pixel_pe = PositionEmbeddingSine(pe_dim // 2, normalize=True)
    ...
    def forward(self, feature_map: torch.Tensor, idrs: torch.Tensor):
        ...
        cur_pe = self.pixel_pe(feature_map)
...
class AFP(nn.Module):
    def __init__(
        ...
        for i in range(self.num_resolutions):
            setattr(
                self,
                f"pixel_pe_{i+1}",
                PositionEmbeddingSine(pe_dim // 2, normalize=True),
            )
            setattr(
                self,
                f"mu_{i+1}",
        ...
        # TODO: j is not used but always setting d1 is wield
    ...
    def forward(
        ...
        for i in range(self.num_resolutions):
            ...
            cur_pe = getattr(self, f"pixel_pe_{i+1}")(feature_map)
