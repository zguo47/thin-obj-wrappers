...
A training script for integrated the essential part of Metric3D on IDisc model training.
A perspective training script based on Metric3D including conversion of any camera input canonical camera and pre-defined resolution.
...
def main_worker(gpu, config: Dict[str, Any], args: argparse.Namespace, ngpus_per_node, run_id: str):
    ...
    for i_d, dataset_name in enumerate(config["data"]["train_dataset"]):
        ...
        if dataset_name == "KITTI360Dataset":
        ...
        elif dataset_name in ["TaskonomyDataset", "HM3DDataset", "HypersimDataset"]:
            train_dataset_single = getattr(custom_dataset, dataset_name)(
                test_mode=False,
                base_path=save_dir_train,
                crop=config["data"]["crop"],
    ...
    elif val_dataset_name in ["TaskonomyDataset", "HM3DDataset", "HypersimDataset"]:
        valid_dataset = getattr(custom_dataset, val_dataset_name)(
            test_mode=True, base_path=save_dir_val, crop=config["data"]["crop"],
            tgt_f=config["data"]["tgt_f"],
            fwd_sz=config["data"]["fwd_sz"],
    ...
    if is_main_process():
        ...
        wandb.init(project="idisc_any_camera", config=config, name=os.path.join(args.checkpoint_path[2:], run_id))
        wandb.watch(model)
...
if __name__ == "__main__":
    ...
    args.checkpoint_path = os.path.join(args.checkpoint_path, os.path.basename(args.config_file[:-5])+"_"+args.model_name)
